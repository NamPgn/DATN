  ...
  return (
    <TailwindComponent>
      <section className="container mx-auto py-8">
        <h1 className={`text-2xl font-bold ${styles.colors.text.primary} mb-8`}>Mã Giảm Giá</h1>
        {isLoading ? (
              ...
              <article key={item.key} className="relative">
                <div className="bg-white rounded-lg overflow-hidden relative">
                  <div className="absolute left-0 top-0 bottom-0 w-4 flex flex-col justify-between">
                    {[...Array(10)].map((_, i) => (
                      <div key={i} className={`w-2 h-2 ${styles.colors.background} rounded-full`} />
                    ))}
                  </div>
                  ...
                  <div className="px-6 py-5">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <small className={`text-[${styles.colors.primary}] text-xs font-semibold uppercase`}>
                          Mã giảm giá
                        </small>
                        <h3 className={`text-lg font-semibold ${styles.colors.text.primary} mt-1`}>
                          {item.code}
                        </h3>
                      ...
                      <div className={`w-14 h-14 bg-[${styles.colors.primary}] rounded-full flex flex-col items-center justify-center text-white font-bold`}>
                        <span className="text-xl leading-none">{item.day}</span>
                        <span className="text-xs uppercase">{item.month}</span>
                      </div>
                    </div>

                    <div className={`border-t border-dashed ${styles.colors.border} pt-4`}>
                      <div className={`flex items-center ${styles.colors.text.secondary} text-sm mb-4`}>
                        <i className={`fa fa-calendar text-[${styles.colors.primary}] mr-2`} />
                        <span>{item.startDate} - {item.expiryDate}</span>
                      </div>
                      ...
                      <div>
                        <button
                          ...
                          style={{
                            backgroundColor: getButtonColor(item.for_logged_in_users, copiedCodes[item.key])
                          }}
                          onClick={() => {
                            if (item.for_logged_in_users == 1) {
